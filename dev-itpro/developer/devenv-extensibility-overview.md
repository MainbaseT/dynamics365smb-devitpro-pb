...
1. **Extending Code Flows**: Codeunits contain AL code that you can reuse across the application. Developers can subscribe to events in base app codeunits and write custom logic to extend the application's functionality. Learn more in [Extending AL code flows: events](devenv-event-types.md).

1. **Extending Tables**: AL developers can add new fields to existing tables or modify the properties of existing fields. This capability allows customization of data structures to fit specific business needs. Learn more in [Extending AL objects: tables](devenv-table-ext-object.md).

1. **Extending Pages**: Pages are the main way to display data in [!INCLUDE[prod_short](../developer/includes/prod_short.md)]. Developers can add or modify fields, actions, and the layout of existing pages or create new ones. Learn more in [Extending AL objects: pages](devenv-page-ext-object.md).

1. **Extending Reports**: Reports provide a structured view of data and are essential for analysis and decision-making. Developers can extend existing reports by adding data items or columns, modifying request pages, or adding new layouts. Learn more in [Extending AL objects: reports](devenv-report-ext-object.md).

1. **Extending the Security Model**: Permission sets define what actions a user can perform in Business Central. Developers can create new permission sets or extend existing ones to ensure users have appropriate access rights. Learn more in [Extending AL objects: security model](devenv-permissionset-ext-object.md).

1. **Extending Enumerations**: Enumerations, or Enums, are lists of constants that represent a set of values. Developers can extend existing Enums or create new ones to cater to unique business requirements. Learn more in [Extending AL objects: enumerations](devenv-extensible-enums.md).

1. **Extending Application Areas**: Application areas help control the visibility of UI elements based on licensing and configuration. Developers can extend these areas to tailor the user interface according to business needs. Learn more in [Extending AL objects: application areas](devenv-extending-application-areas.md).
...
* The System Application, which is a set of open-source modules that make it easier to build, maintain, and upgrade on-premises and online apps. These modules let you focus on the business logic and the needs of your users or customers.
...
If you want to contribute code to the Business Central open-source System Application, determine the type of contribution:

1. Small bug fixes, product improvements, or customer conveniences; or
1. New capabilities or larger changes to the existing application platform.

For more information about making contributions, see [Contributing to the Business Central open source System Application](https://github.com/microsoft/BCApps/blob/main/CONTRIBUTING.md).

To learn more about the System Application, its module architecture, and how to modify or create new modules, see [Creating new modules in the system application](devenv-blueprint.md).
...
Numerous examples demonstrate how to extend various features in [!INCLUDE[prod_short](../developer/includes/prod_short.md)]:

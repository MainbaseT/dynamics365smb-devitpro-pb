...
The following objects are marked as `ObsoleteState = Pending` in version 28.3 and will be removed in a future version.

### Subcontracting Worksheet moved to the Subcontracting app

|Moved, Removed, or Replaced? |    Why?|
|-----------------------------|-----|
|Moved| The following objects in the base application are moved to the **Subcontracting** app, which ships as a separate extension:<ul><li>**Subcontracting Worksheet** page (replaced by page "Subc. Subcontracting Worksheet" in the Subcontracting app)</li><li>**Calculate Subcontracts** report (replaced by report "Subc. Calculate Subcontracts" in the Subcontracting app)</li><li>**"For. Labor"** enum value on the Requisition Worksheet Template Type (replaced by the Subcontracting app)</li></ul>Related navigation actions on the following role centers are also obsoleted:<ul><li>Production Planner</li><li>Shop Supervisor</li><li>Manufacturing Manager</li><li>Purchasing Agent</li></ul>The Subcontracting app provides the same functionality with additional capabilities, including WIP transfers, location management, purchase provisions, and item charges for subcontracting. Partners with extensions that reference the obsoleted objects should take a dependency on the Subcontracting app and update references to the new object names (prefixed with "Subc.").</br></br>These objects are marked as `ObsoleteState = Pending` in version 28.3 and will be removed in a future version.|

Learn more about the Subcontracting app in [Subcontracting overview](/dynamics365/business-central/production-how-to-subcontract-manufacturing).
...
|Moved, Removed, or Replaced? |    Why?|
|-----------------------------|-----|
|Replaced| The following Excel reports are removed on the **Business Manager** and **Accountant** role centers.<ul><li>Balance Sheet</li><li>Income Statement</li><li>Statement of Cash Flow</li><li>Statement of Retained Earnings</li><li>Sales Taxes Collected</li><li>Customer Statements</li><li>Aged Accounts Payable</li><li>Aged Accounts Receivable</li></ul>Although these reports are useful to some organizations, changing them requires technical skills in AL and VBA (Visual Basic for Applications) programming. Over the last few years, each release wave has enhanced analytics capabilities in finance. For example, the enhancements include data analysis capabilities on ledger tables, reports with Excel layouts that you can change the layout as you see fit, and Power BI reports for finance. </br></br>Learn more about the replacement features in [aka.ms/bcFinanceAnalytics](https://aka.ms/bcFinanceAnalytics). |

### Legacy Power BI apps on Marketplace (warning)

|Moved, Removed, or Replaced? |    Why?|
|-----------------------------|-----|
|Replaced| While useful for showing the Power BI embed capabilities in [!INCLUDE [prod_short](../developer/includes/prod_short.md)], the following legacy Power BI apps on Marketplace are replaced: <ul><li>Dynamics 365 Business Central - CRM</li><li>Dynamics 365 Business Central - Finance</li><li>Dynamics 365 Business Central - Sales</li></ul> The apps are replaced by seven Power BI apps for finance, sales, purchasing, inventory, projects, and manufacturing. The new Power BI apps were released in version 25.1. </br></br>Learn more about the replacement features in [aka.ms/bcFinanceAnalytics](https://aka.ms/bcFinanceAnalytics).  |
...
|Moved, Removed, or Replaced? |    Why?|
|-----------------------------|-----|
|Replaced| Businesses in all countries and regions can benefit from the ability to review general ledger entries before posting them. In a future release, Microsoft deprecates the country-specific feature and replaces it with one that's available in all country versions. After the feature is deprecated, you can use it to access previous reviews, but not to do new reviews. Microsoft archives your data according to local requirements. Learn more about the replacement feature in [Review Amounts in General Ledger Accounts](/dynamics365/business-central/finance-review-accounts).  |
